...
private:
	...
	void declareResources() {
		for (const rtsl::ir::Resource& resource : module.resources) {
			...
			if (resource.kind == rtsl::ir::ResourceKind::resource_sampled_texture) instruction(59, {pointerType(0, typeSampledImage2D()), variable, 0});
				...
				const rtsl::ir::Type* type = module.findType(resource.type); const std::string_view name = resourceTypeName(resource); const std::uint32_t extent = name == "image_1d" ? 1 : name == "image_2d" ? 2 : name == "image_3d" ? 3 : 0;
				if (!type || type->parameter_types.size() != 1 || !extent) throw std::runtime_error("RTIR storage image must preserve image dimension and texel type");
	...
	void emitIRInstruction(const rtsl::ir::Instruction& source,
		...
		case rtsl::ir::Opcode::opcode_resource_load: {
			...
			if (resource_variable != resource_variables.end()) {
				...
				if (resource.kind == rtsl::ir::ResourceKind::resource_storage_buffer) {
					...
					const std::uint32_t member = source.operands.empty() || module.findType(resource_type->parameter_types[0])->kind == rtsl::ir::TypeKind::type_void ? 0 : 1; std::vector<std::uint32_t> chain{pointerType(12, typeFor(source.type)), id(), resource_variable->second, constantUnsignedInteger(member)}; if (!source.operands.empty()) chain.push_back(valueFor(values, source.operands[0])); instruction(65, chain); result = id(); instruction(61, {typeFor(source.type), result, chain[1]}); break;
				}
				if (resource.kind == rtsl::ir::ResourceKind::resource_storage_texture) {
					const std::string_view name = resourceTypeName(resource); const std::size_t extent = name == "image_1d" ? 1 : name == "image_2d" ? 2 : name == "image_3d" ? 3 : 0;
					...
					const std::uint32_t image = id(); instruction(61, {typeStorageImage(resource_type->parameter_types[0], (std::uint32_t)extent), image, resource_variable->second}); const std::uint32_t physical = id(); instruction(98, {typeVector(typeFloat(), 4), physical, image, coordinate}); result = id(); instruction(81, {typeFor(source.type), result, physical, 0}); break;
				}
		...
		case rtsl::ir::Opcode::opcode_resource_store: {
			...
			if (resource.kind == rtsl::ir::ResourceKind::resource_storage_buffer) { if (source.operands.size() != 2) throw std::runtime_error("RTIR buffer resource store requires coordinate and value"); const std::uint32_t pointer = id(); const auto type = value_types.at(source.operands[1].value()); instruction(65, {pointerType(12, typeFor(type)), pointer, resource_variable->second, constantUnsignedInteger(module.findType(module.findType(resource.type)->parameter_types[0])->kind == rtsl::ir::TypeKind::type_void ? 0 : 1), valueFor(values, source.operands[0])}); instruction(62, {pointer, valueFor(values, source.operands[1])}); break; }
			if (resource.kind == rtsl::ir::ResourceKind::resource_storage_texture) { const std::string_view name = resourceTypeName(resource); const std::size_t extent = name == "image_1d" ? 1 : name == "image_2d" ? 2 : name == "image_3d" ? 3 : 0; if (!extent || source.operands.size() != extent + 1) throw std::runtime_error("RTIR image resource store has the wrong coordinate count"); std::vector<std::uint32_t> coordinates; for (std::size_t index = 0; index < extent; ++index) { const std::uint32_t component = id(); instruction(124, {typeInteger(32, true), component, valueFor(values, source.operands[index])}); coordinates.push_back(component); } const std::uint32_t coordinate = extent == 1 ? coordinates[0] : [&] { const std::uint32_t value = id(); std::vector<std::uint32_t> construct{typeVector(typeInteger(32, true), (std::uint32_t)extent), value}; construct.insert(construct.end(), coordinates.begin(), coordinates.end()); instruction(80, construct); return value; }(); const rtsl::ir::Type* type = module.findType(resource.type); const std::uint32_t image = id(); instruction(61, {typeStorageImage(type->parameter_types[0], (std::uint32_t)extent), image, resource_variable->second}); const std::uint32_t value = valueFor(values, source.operands.back()); const std::uint32_t physical = id(); instruction(80, {typeVector(typeFloat(), 4), physical, value, constantFloat(0.0f), constantFloat(0.0f), constantFloat(1.0f)}); instruction(99, {image, coordinate, physical}); break; }
			throw std::runtime_error("RTIR resource store uses an unsupported resource kind");
		}
		case rtsl::ir::Opcode::opcode_resource_query: {
			if (source.immediates.size() != 1 || !source.type || !source.operands.empty()) throw std::runtime_error("RTIR resource query is malformed"); const auto resource_variable = resource_variables.find(source.immediates[0]); const rtsl::ir::Resource& resource = requireResource(source.immediates[0]); if (resource_variable == resource_variables.end() || resource.kind != rtsl::ir::ResourceKind::resource_storage_texture) throw std::runtime_error("RTIR resource query requires a storage image"); const rtsl::ir::Type* image_type = module.findType(resource.type); const rtsl::ir::Type* result_type = module.findType(source.type); const std::uint32_t extent = result_type && result_type->kind == rtsl::ir::TypeKind::type_vector ? result_type->element_count : 1; const std::uint32_t signed_type = extent == 1 ? typeInteger(32, true) : typeVector(typeInteger(32, true), extent); const std::string_view name = resourceTypeName(resource); const std::uint32_t image = id(); instruction(61, {typeStorageImage(image_type->parameter_types[0], name == "image_1d" ? 1 : name == "image_2d" ? 2 : 3), image, resource_variable->second}); const std::uint32_t queried = id(); instruction(104, {signed_type, queried, image}); result = id(); instruction(124, {typeFor(source.type), result, queried}); break;
		}
		default:
