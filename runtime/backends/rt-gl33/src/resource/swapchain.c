...
struct rtgl_swapchain* rtgl_swapchain_create(struct rtgl_context* ctx) {
	...
	RTGL_CHECK_ALLOC(swapchain, sizeof(*swapchain), "GL33 swapchain");
	if (swapchain) {
		rtgl_printf("rt-gl33: swapchain created\n");
	}
	return swapchain;
}
...
void rtgl_swapchain_destroy(struct rtgl_context* ctx, struct rtgl_swapchain* swapchain) {
	...
	rtgl_printf("rt-gl33: swapchain destroyed\n");
	free(swapchain);
}
...
void rtgl_swapchain_init_from_window(struct rtgl_context* ctx, struct rtgl_swapchain* swapchain, native_window_handle_t window, u32 width, u32 height) {
	...
	if (swapchain->bound) {
		rtgl_throwf(RT_IMPROPER_USAGE, "GL33 swapchain is already bound to a window");
		return;
	}
	if (width == 0 || height == 0) {
	...
	rtgl_printf("rt-gl33: swapchain bound to window (%ux%u)\n", width, height);
}
...
void rtgl_swapchain_init_from_surface(struct rtgl_context* ctx, struct rtgl_swapchain* swapchain, struct gl_surface* surface, u32 width, u32 height) {
	...
	swapchain->height = height;
	swapchain->bound = true;
	rtgl_printf("rt-gl33: swapchain bound to surface (%ux%u)\n", width, height);
}
