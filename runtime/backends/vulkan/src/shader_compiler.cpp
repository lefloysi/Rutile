...
static std::unordered_map<u32, std::string> rtvk_rtslp_struct_names(const rt::RTArtifactModule& module) {
	// The RTSLP type pool can contain synthetic empty TypeStructs in addition
	// to the user-declared structs (e.g. constructor placeholders), so we
	// can't pair pool entries with module.structs by pool-order index alone.
	// Instead, walk module.structs in declaration order and pair each one
	// with the next *unused* TypeStruct in the pool whose operand count
	// matches the struct's field count.
	...
	for (const auto& decl : module.structs) {
		...
		for (const auto& inst : module.type_constant_pool) {
			if (inst.op != rt::RTIROp::TypeStruct) continue;
...
static bool rtvk_rtslp_emit_stage_spirv(const rt::RTArtifactModule& module, rt::RTStageKind stage, std::vector<u32>* spirv_out) {
	...
	for (const auto& inst : fn->body) {
		...
		// OpVariable's first literal is a SPIR-V storage class. The RTSLP IR
		// uses its own storage-class numbering (Function=0, Input=1, ...), so
		// it must go through rtvk_rtslp_storage_class() before emission.
		// Other ops pass their literals through verbatim.
		if (inst.op == rt::RTIROp::Variable && !inst.literals.empty()) {
			words.push_back(rtvk_rtslp_storage_class(inst.literals[0]));
