...
struct rtval_graphics_program* rtval_graphics_program_create(void) {
	...
	if (!backend) { rtval_report_error("rtGraphicsProgramCreate"); return NULL; }
	struct rtval_graphics_program* handle = rtval_handle_create(RTVAL_HANDLE_TYPE_GRAPHICS_PROGRAM);
	if (!handle) { rtval_next_rtGraphicsProgramDestroy(backend); return NULL; }
...
void rtval_graphics_program_destroy(struct rtval_graphics_program* program) {
	...
	if (!state) { RTVAL_DROP("rtGraphicsProgramDestroy: invalid handle"); return; }
...
void rtval_graphics_program_vertex_layout(struct rtval_graphics_program* program, const rt_vertex_layout* layout) {
	struct rtval_graphics_program* state = RTVAL_PAYLOAD(program, struct rtval_graphics_program);
	if (!state) { RTVAL_DROP("rtGraphicsProgramVertexLayout: invalid handle"); return; }
	if (layout && layout->attribute_count && !layout->attributes) { RTVAL_DROP("rtGraphicsProgramVertexLayout: missing attributes"); return; }
	if (layout && layout->attribute_count && layout->stride == 0) { RTVAL_DROP("rtGraphicsProgramVertexLayout: zero stride"); return; }
...
void rtval_graphics_program_vertex_shader(struct rtval_graphics_program* program, u64 size, const void* data) {
	struct rtval_graphics_program* state = RTVAL_PAYLOAD(program, struct rtval_graphics_program);
	if (!state) { RTVAL_DROP("rtGraphicsProgramVertexShader: invalid handle"); return; }
	if (!data || size == 0) { RTVAL_DROP("rtGraphicsProgramVertexShader: empty shader data"); return; }
...
void rtval_graphics_program_fragment_shader(struct rtval_graphics_program* program, u64 size, const void* data) {
	struct rtval_graphics_program* state = RTVAL_PAYLOAD(program, struct rtval_graphics_program);
	if (!state) { RTVAL_DROP("rtGraphicsProgramFragmentShader: invalid handle"); return; }
	if (!data || size == 0) { RTVAL_DROP("rtGraphicsProgramFragmentShader: empty shader data"); return; }
...
void rtval_graphics_program_raster_state(struct rtval_graphics_program* program, enum rt_cull_mode cull_mode, enum rt_front_face front_face, enum rt_fill_mode fill_mode) {
	struct rtval_graphics_program* state = RTVAL_PAYLOAD(program, struct rtval_graphics_program);
	if (!state) { RTVAL_DROP("rtGraphicsProgramRasterState: invalid handle"); return; }
...
void rtval_graphics_program_blend_state(
	...
	struct rtval_graphics_program* state = RTVAL_PAYLOAD(program, struct rtval_graphics_program);
	if (!state) { RTVAL_DROP("rtGraphicsProgramBlendState: invalid handle"); return; }
...
void rtval_graphics_program_link(struct rtval_graphics_program* program) {
	struct rtval_graphics_program* state = RTVAL_PAYLOAD(program, struct rtval_graphics_program);
	if (!state) { RTVAL_DROP("rtGraphicsProgramLink: invalid handle"); return; }
...
rt_uniform_location rtval_graphics_program_uniform_location(struct rtval_graphics_program* program, const char* name) {
	struct rtval_graphics_program* state = RTVAL_PAYLOAD(program, struct rtval_graphics_program);
	if (!state) { RTVAL_DROP("rtGraphicsProgramUniformLocation: invalid handle"); return RT_NULL_HANDLE; }
	if (!name) { RTVAL_DROP("rtGraphicsProgramUniformLocation: NULL name"); return RT_NULL_HANDLE; }
