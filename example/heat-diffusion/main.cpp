...
void update_camera(GLFWwindow* window, AppState* state, Camera* camera, Camera* target_camera, f32 dt) {
	...
	f32 dx = 0.0f;
	f32 dy = 0.0f;
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) { dx -= 1.0f; }
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) { dx += 1.0f; }
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) { dy += 1.0f; }
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) { dy -= 1.0f; }
	if (dx != 0.0f || dy != 0.0f) {
...
int main(int argc, char* argv[]) {
	...
	GLFWwindow* window = glfwCreateWindow(kWindowWidth, kWindowHeight, "Rutile Heat Diffusion", nullptr, nullptr);
	if (!window) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (fps_delta.count() >= 0.5f) {
			...
			std::snprintf(title, sizeof(title), "Rutile Heat Diffusion - %.0f FPS - %.1fx - left mouse draws",
				fps, params.camera[2]);
